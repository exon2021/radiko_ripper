...
begin
  ...
  begin
      ...
      begin
          ...
          writeln('rtmpdump front-end "radiko_ripper" programed by exon@2021');
          writeln('<how to use>');
          ...
          writeln('rtmpdump実行中はいつでもCTRL+Cで中止できます');
          writeln(' ');
...

{
  ...
  //大事　wgetに渡す引数で、リトライやタイムアウトの設定は入れてはいけない
  //設定すると処理が戻らずゴーストプロセスになる

{  //デバッグ用　バッチファイル
  strList := TStringList.Create;
  strList.Add('wget ' + strA);
  strList.SaveToFile('test.bat');
  strList.Free;
}
